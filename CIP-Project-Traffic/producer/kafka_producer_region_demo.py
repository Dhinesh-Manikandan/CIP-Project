...
# ================= BOSTON CORRIDOR BOUNDING BOX =================
# Focus: Massachusetts Ave Corridor – Boston
MIN_LAT = 42.30
MAX_LAT = 42.37
MIN_LNG = -71.12
MAX_LNG = -71.05
...
def main():
    ...
    try:
        for chunk in pd.read_csv(
            ...
            # 🔥 Filter Boston corridor
            chunk = chunk[
                (chunk["Start_Lat"] >= MIN_LAT) &
                (chunk["Start_Lat"] <= MAX_LAT) &
                (chunk["Start_Lng"] >= MIN_LNG) &
                (chunk["Start_Lng"] <= MAX_LNG)
            ]

            if chunk.empty:
                continue

            # Rename for cleaner downstream usage
            chunk = chunk.rename(columns={
                "Visibility(mi)": "Visibility",
                "DelayFromTypicalTraffic(mins)": "Delay"
            })
            ...
            producer.flush()
            batch += 1
            logging.info(f"✅ Sent Boston batch {batch} ({len(chunk)} records)")
            time.sleep(3)
